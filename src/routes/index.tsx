...
export const Route = createFileRoute("/")({
  loader: async () => {
    ...
    try {
      ...
      novedades = todosLosProductos
        .filter(
          (p) =>
            p.tags?.includes("novedad") || p.tags?.includes("edicion_limitada")
        )
        .slice(0, 6);
    } catch {
      novedades = [];
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Flores y regalos a domicilio en Lima. Delivery en Miraflores, Surco, Barranco, Lince y San Isidro." },

...
import {
  Plus, Trash2, ChevronUp, ChevronDown, Loader2, X, Star,
  Cake, Heart, Gem, Flower2, GraduationCap, Briefcase,
  Baby, Gift, Sun, TreePine, Music
} from "lucide-react";
...
interface AgregarModalProps {
  ...
  onClose: () => void;
  onGuardar: (data: { nombre: string; icono: string; categoria_id: string | null }) => Promise<void>;
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white w-full max-w-md shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block font-body text-xs tracking-widest uppercase text-[#8A7A6E] mb-1.5">Nombre *</label>
            <input
              value={nombre}
          ...
          <div>
            <label className="block font-body text-xs tracking-widest uppercase text-[#8A7A6E] mb-1.5">Ícono</label>
          ...
          <div>
            <label className="block font-body text-xs tracking-widest uppercase text-[#8A7A6E] mb-1.5">Categoria vinculada</label>
...
function OcasionesPage() {
  ...
  async function handleToggle(oc: OcasionHomeRow) {
    const { error: err } = await supabase.from("ocasiones_home").update({ activo: !oc.activo }).eq("id", oc.id);
  ...
  async function handleMove(idx: number, dir: -1 | 1) {
    ...
    if (resA.error || resB.error) { alert("Error al reordenar"); return; }
  ...
  async function handleAgregar(data: { nombre: string; icono: string; categoria_id: string | null }) {
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="font-body text-sm text-[#8A7A6E] mt-1">{ocasiones.length} ocasion{ocasiones.length !== 1 ? "es" : ""}</p>
          </div>
          <button
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-[#E8DDD0] bg-[#F5EFE6]">
                <th className="px-4 py-3 text-left font-body text-[10px] tracking-widest uppercase text-[#8A7A6E]">Orden</th>
                <th className="px-4 py-3 text-left font-body text-[10px] tracking-widest uppercase text-[#8A7A6E]">Nombre</th>
                <th className="px-4 py-3 text-left font-body text-[10px] tracking-widest uppercase text-[#8A7A6E]">Icono</th>
                <th className="px-4 py-3 text-left font-body text-[10px] tracking-widest uppercase text-[#8A7A6E]">Activo</th>
                <th className="px-4 py-3 text-left font-body text-[10px] tracking-widest uppercase text-[#8A7A6E]">Eliminar</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                [1,2,3].map((n) => (
                  <tr key={n} className="border-b border-[#E8DDD0] animate-pulse">
                    <td className="px-4 py-3"><div className="h-4 w-8 bg-[#F5EFE6] rounded" /></td>
                    ...
                    <td className="px-4 py-3"><div className="h-5 w-10 bg-[#F5EFE6] rounded" /></td>
                    <td className="px-4 py-3"><div className="h-4 w-6 bg-[#F5EFE6] rounded" /></td>
                  </tr>
                ))
              ) : ocasiones.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-10 text-center font-body text-sm text-[#8A7A6E]">No hay ocasiones. Agrega una.</td></tr>
              ) : (
                ocasiones.map((oc, idx) => (
                  <tr key={oc.id} className="border-b border-[#E8DDD0] hover:bg-[#FDFAF6] transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-0.5">
                        <button onClick={() => handleMove(idx, -1)} disabled={idx === 0} className="text-[#8A7A6E] hover:text-[#2C2420] disabled:opacity-30 transition-colors">
                          <ChevronUp className="h-4 w-4" strokeWidth={1.5} />
                        </button>
                        <button onClick={() => handleMove(idx, 1)} disabled={idx === ocasiones.length - 1} className="text-[#8A7A6E] hover:text-[#2C2420] disabled:opacity-30 transition-colors">
                          <ChevronDown className="h-4 w-4" strokeWidth={1.5} />
                        </button>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      ...
                      >
                        <span className={`inline-block h-3.5 w-3.5 rounded-full bg-white shadow transition-transform ${oc.activo ? "translate-x-[18px]" : "translate-x-[3px]"}`} />
                      </button>
                    </td>
                    <td className="px-4 py-3">
                      <button onClick={() => handleDelete(oc.id)} className="text-[#8A7A6E] hover:text-red-500 transition-colors">

    ...
    <header className="bg-ivory">
      <div className="px-6 md:px-10 h-20 grid grid-cols-3 items-center">
        <div className="flex items-center">
          <button aria-label="Abrir menu" onClick={() => setOpen(true)} className="md:hidden p-2 -ml-2 hover:opacity-60 transition-opacity">
            <Menu className="h-6 w-6" strokeWidth={1.25} />
          </button>
          <button aria-label="Buscar" className="hidden md:inline-flex p-2 -ml-2 hover:opacity-60 transition-opacity">
        ...
        <Link to="/" className="flex flex-col items-center justify-center text-center leading-none">
          <span className="font-display italic font-bold text-3xl md:text-4xl tracking-tight">Miraflores</span>
          <span className="mt-1 text-[10px] tracking-widest uppercase font-body font-light text-muted-foreground">boutique floral</span>
        </Link>

        <div className="flex items-center justify-end gap-1">
          <a href="https://wa.me/51999600482" aria-label="WhatsApp" className="p-2 hover:opacity-60 transition-opacity" target="_blank" rel="noopener noreferrer">
            <WhatsappIcon className="h-5 w-5" />
          </a>
          <button aria-label={`Carrito${itemCount > 0 ? ` (${itemCount} items)` : ""}`} onClick={abrirCarrito} className="relative p-2 -mr-2 hover:opacity-60 transition-opacity">
      ...
      <nav className="hidden md:block border-t border-b border-border">
        <ul className="px-6 md:px-10 flex gap-6 md:gap-8 justify-center items-center h-12 whitespace-nowrap text-[12px] tracking-widest uppercase font-body font-light">
          <li>
            <Link to="/" className="text-foreground hover:opacity-70 transition-opacity">Inicio</Link>
          </li>
          {padres.map((c) => (
            <li key={c.slug}>
              <Link to="/categoria/$slug" params={{ slug: c.slug }} className="text-muted-foreground hover:text-foreground transition-colors" activeProps={{ className: "text-foreground" }}>
                {c.nombre}
              </Link>
            </li>
          ))}
      ...
      {open && (
        <>
          <div className="md:hidden fixed inset-0 bg-foreground/40 z-40" onClick={close} aria-hidden />
          <div className="md:hidden fixed top-0 left-0 bottom-0 w-[86%] max-w-sm bg-background z-50 shadow-xl flex flex-col">
            <div className="flex items-center justify-between h-16 px-5 border-b border-border">
              ...
              <button aria-label="Cerrar" onClick={close} className="p-2 -mr-2 text-foreground hover:opacity-70">
                ...
                return (
                  <div key={c.id} className="border-b border-border/60">
                    <div className="flex items-stretch">
                      <Link to="/categoria/$slug" params={{ slug: c.slug }} onClick={close} className="flex-1 px-5 py-4 font-display text-foreground text-lg">
                        {c.nombre}
                      </Link>
                      {hijas.length > 0 && (
                        <button aria-label={`Expandir ${c.nombre}`} aria-expanded={isOpen} onClick={() => setOpenCat(isOpen ? null : c.id)} className="px-5 text-muted-foreground hover:text-foreground">
                          <ChevronDown className={`h-5 w-5 transition-transform ${isOpen ? "rotate-180" : ""}`} strokeWidth={1.5} />
                        </button>
                      )}
                    </div>
                    {hijas.length > 0 && isOpen && (
                      <ul className="bg-ivory-soft/60 pb-2">
                        {hijas.map((s) => (
                          <li key={s.id}>
                            <Link to="/categoria/$slug/$sub" params={{ slug: c.slug, sub: s.slug }} onClick={close} className="block px-8 py-3 text-[13px] font-body font-light text-foreground/80 hover:text-foreground">
                              {s.nombre}
                            </Link>
                          </li>
                        ))}

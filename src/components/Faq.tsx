...
export function Faq() {
  return (
    <section id="faq" className="px-5 md:px-12 lg:px-16 py-16 md:py-24 bg-[#FAF8F5]/50">
      <div className="max-w-[1536px] mx-auto">
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-16 items-start">
          <div className="lg:col-span-5">
            ...
            <p className="mt-4 md:mt-6 font-body font-light text-[#2C2420]/75 max-w-md text-sm sm:text-base leading-relaxed">
              Descubra todo lo que necesita saber para asegurar una experiencia de compra impecable en nuestra boutique floral. Si requiere asistencia inmediata, nuestro equipo está siempre disponible por WhatsApp.
            </p>
          </div>
            ...
            <Accordion type="single" collapsible className="w-full space-y-1">
              {FAQS_PREDETERMINADAS.map((item, i) => (
                <AccordionItem
                  key={item.id}
                  value={`item-${i}`}
                  className="border-b border-[#E8DDD0]/80 py-1"
                >
                  <AccordionTrigger className="text-left font-display text-base sm:text-lg md:text-xl font-normal text-[#2C2420] hover:text-[#8A7A6E] hover:no-underline py-4 md:py-5 transition-colors">
                    {item.pregunta}
                  </AccordionTrigger>
                  <AccordionContent className="font-body font-light text-[#2C2420]/75 text-sm sm:text-base pb-5 leading-relaxed">
                    {item.respuesta}
                  </AccordionContent>
                </AccordionItem>
              ))}
